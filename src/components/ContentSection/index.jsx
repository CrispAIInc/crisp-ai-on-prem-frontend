...
const UpdateFilenameModal = ({
    ...
    const { theme, setDisplayedSources, categoryOptions, setKnowledgeBase } =
    ...
    async function updateFilename() {
        try {
            if (filename === "") {
                toast("filename cannot be empty", {
            ...
            onHide();
            toast("Source renamed successfully", {
        ...
        >
            ...
            >
                <div className="flex flex-col">
                    <label
                        htmlFor="indexName"
                        className={`block text-sm font-medium ${theme === "dark" && "text-gray-300"
                            }`}
                    >
                        Rename source
                    </label>
                    <div className="flex items-center gap-1">
                        ...
                        />
                        <span
                            className={`${theme === "light" ? "text-textColor-100" : "text-textColor-200"
                                }`}
                        >
                            .{extension}
                        </span>
            ...
            <Modal.Footer
                className={`flex items-center gap-3 ${theme === "light"
                    ? ""
                    : "!bg-textColor-300 !text-white !border-t !border-t-textColor-200"
                    }`}
            >
                <div
                    className={`flex items-center justify-center gap-2  rounded-md cursor-pointer w-fit ${theme === "light"
                ...
                >
                    <span
                        className={`select-none font-medium ${theme === "light" ? "text-textColor-300" : "text-textColor-100"
                            }`}
                    >
                        Cancel
                    </span>
...
const ContentSection = ({
    ...
    const deleteResource = async (event, items) => {
        try {
            ...
            setDisplayedSources((prev) =>
                prev.filter((item) => item.source_path !== items[0].source_path)
            );
            ...
            let updatedKnowledgeBase = data.map((item) => {
                let selected = sourcesTobeCommited.find(
    ...
    const handleUpload = async (event, fileFormat, _files) => {
        try {
            ...
            await makeApiRequest("/upload", "post", formData, {
                "Content-type": "multipart/form-data",
            });

            toast("Upload complete. Generate metadata from the right panel", {
                className: "p-2 rounded-md",
                theme,
            });
            ...
            setKnowledgeBase(
                data.map((item) => ({
                    ...item,
                    is_selected:
                        sourcesToAdd.some((s) => s.source_path === item.source_path) ||
                        sourcesTobeCommited.find((i) => i.source_path === item.source_path)
                            ?.is_selected,
                }))
            );
            ...
            setDisplayedSources((prev) => {
                const newSources = sourcesToAdd.filter(
                    (item) => !prev.some((i) => i.source_path === item.source_path)
                );
                return [
                    ...prev,
                    ...newSources.map((item) => ({ ...item, is_selected: true })),
                ];
            });
    ...
    const handleSelectAllCheckboxChange = (path, isChecked) => {
        ...
        if (category === undefined) {
            ...
            setKnowledgeBase(updatedKnowledgeBase);
            setSourcesTobeCommited(
                updatedKnowledgeBase.filter((item) => item.is_selected)
            );
        } else if (category !== undefined && format !== undefined) {
            const updatedKnowledgeBase = knowledgeBase.map((item) => {
                if (
                    (category === "all" || item.category.includes(category)) &&
                    (item.file_type === format || format === "all")
                ) {
                    item.is_selected = isChecked;
                }
                return item;
    ...
    function handleToggleCheckSources(isChecked) {
        if (isChecked) {
            setDisplayedSources((prev) =>
                prev.map((item) => ({ ...item, is_selected: true }))
            );
            // update knowledgebase depending on the items selected in displayedSources
            const updatedKnowledgeBase = knowledgeBase.map((prev) => {
                let itemExist = displayedSources?.find(
                    (i) => i?.source_path === prev?.source_path
                );
                if (itemExist) {
                    return { ...prev, is_selected: true };
                }
            ...
            setSourcesTobeCommited([]);
            setDisplayedSources((prev) =>
                prev.map((item) => ({ ...item, is_selected: false }))
            );
        }
    }
    ...
    const [isUpdateFilenameModalOpen, setIsUpdateFilenameModalOpen] =
        useState(false);
                ...
                <section className={`relative flex flex-col items-start h-full`}>
                    <div className="w-full">
                        <div className="w-full max-w-4xl pr-3">
                            {/* home */}
                            {/* <div
                            className={`source-explorer flex mb-1 items-center justify-center gap-2 px-2 py-2 rounded-md cursor-pointer w-fit ${theme === 'light' ? 'hover:bg-textColor-100/40' : 'hover:bg-light-hover-200/20'}`}
                        >
                            ...
                            <div className="flex flex-col justify-start gap-2 mb-1">
                                ...
                                <div className="flex flex-wrap items-center gap-0">
                                    <div
                                        className={`source-explorer flex items-center justify-center gap-2 px-2 py-2 rounded-md cursor-pointer w-fit ${theme === "light"
                        ...
                        {/* Settings */}
                        {/* <div
                            className={`source-explorer flex items-center justify-center gap-2 px-2 py-2 rounded-md cursor-pointer w-fit ${theme === 'light' ? 'hover:bg-textColor-100/40' : 'hover:bg-light-hover-200/20'}`}
                        >
                    ...
                    <div className="flex flex-col flex-1 w-full h-full max-h-full overflow-y-auto">
                        <BaseHeading
                            text={`Sources (${displayedSources?.length} selected & ${displayedSources?.filter((i) => i?.is_selected)?.length
                                } checked.)`}
                            className="mt-4"
                        />
                        ...
                        <div className="flex flex-col flex-1 w-full h-full overflow-y-hidden selected-sources-container">
                            {displayedSources?.length > 0 && (
                                <div
                                    className={` h-full gap-2  w-full max-w-full mt-4 overflow-y-auto ${theme === "dark"
                                        ? "!border !border-textColor-300"
                                        : "border"
                                        } empty:!border-none`}
                                >
                                    ...
                                    {displayedSources
                                        ...
                                        .map((option) => (
                                            <div
                                                key={option?.source_path}
                                                className={`flex w-full max-w-full cursor-pointer py-2 px-1 ${showSourceContextMenu === null &&
                                                    (theme === "light"
                                                        ? "hover:bg-textColor-100/40"
                                                        : "hover:bg-light-hover-200/20")
                                                    }`}
                                                onMouseEnter={() =>
                                                    handleMouseEnter(option?.source_path)
                                                }
                                                onMouseLeave={handleMouseLeave}
                                                onClick={(event) => onThumbnailClick(event, option)}
                                            >
                                                <div className="relative flex items-center flex-1 w-full max-w-full gap-2">
                                                    {showSourceContextMenu === option?.source_path && (
                                                        <div
                                                            ref={dropdownRef}
                                                            className={` absolute left-0 top-full z-10 flex flex-col items-center  p-1 rounded-md shadow-lg ${theme === "dark" ? "bg-gray-900" : "bg-white"
                                                                }`}
                                                        >
                                                            <div
                                                                className={`flex  gap-2  py-2 pr-10 pl-1 font-medium text-left ${theme === "light"
                                                                    ? "hover:bg-textColor-100/40"
                                                                    : "text-textColor-100 hover:bg-slate-800/50"
                                                                    }`}
                                                                onClick={(event) =>
                                                                    handleOpenFilenameUpdateModal(event, option)
                                                                }
                                                            >
                                                                <EditOutlinedIcon
                                                    ...
                                                    <span
                                                        className={`text-md font-medium break-all ${theme === "dark" && "text-textColor-100"
                                                            }`}
                                                    >
                                                        {option.source_path.replace(/\.[^/.]+$/, "")}
                                                    </span>
                                                </div>
                    ...
                    {isUpdateFilenameModalOpen && (
                        <UpdateFilenameModal
                            show={isUpdateFilenameModalOpen}
                            onHide={() => setIsUpdateFilenameModalOpen(false)}
                            filename={filename}
                            extension={updatingSource?.source_path?.split(".")?.at(-1)}
                            setFilename={setFilename}
                            oldFilename={updatingSource?.source_path}
                            sourceCategory={updatingSource?.category}
                            filetype={updatingSource?.file_type}
                        />
                    )}
