...
export default function IndexSearchBar({
    ...
    const toggleIndex = (id) => {
        setSelectedIndexes(
            selectedIndexes.includes(id) ? selectedIndexes.filter((i) => i !== id) : [...selectedIndexes, id]
        );
    };
    ...
    return (
        <div ref={rootRef} className="relative">
            <div className="flex items-stretch gap-1.5 rounded-xl bg-gradient-to-r from-primary/10 to-accent/10 border border-border p-1">
                <div className="flex-1 flex flex-wrap items-center gap-1.5 bg-surface rounded-[10px] pl-2.5 pr-1.5 py-1.5 min-h-[38px]">
                    {selectedIndexes.map((value) => {
                        ...
                        return (
                            <span
                                key={value}
                                className="inline-flex items-center gap-1 bg-surface-alt text-ink text-[12px] font-medium pl-2.5 pr-1.5 py-1 rounded-lg shrink-0"
                            >
                                {opt?.label ?? value}
                                <button
                                    type="button"
                ...
                <button
                    ...
                    className="shrink-0 rounded-[10px] px-4 text-[12.5px] font-bold text-white bg-gradient-to-r from-primary/80 to-accent/80 hover:brightness-105 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-1.5"
                >
                    <Search size={13} />
                    Discover
                </button>
