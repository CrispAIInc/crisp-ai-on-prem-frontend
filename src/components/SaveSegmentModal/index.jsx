...
const SaveSegmentModal = ({ show, onHide, segment, setSegmentDescriptions, onSaved }) => {
    ...
    const { theme, setCurrentSegment } = useContext(MainContext);
        ...
        <Modal show={show} onHide={onHide} size="md" aria-labelledby="save-segment" centered dialogClassName='text-left'>
            <Modal.Header className={`border-0 pb-0`}>
                <div className="flex flex-col gap-1">
                    <Modal.Title id="save-segment" className={`text-lg font-semibold text-gray-900`}>
                        Save segment
                    </Modal.Title>
            ...
            <Modal.Body>
                <div className='flex flex-col items-start justify-center gap-3'>
                    <div className="flex flex-col w-full">
                        <label htmlFor="segmentTitle" className={`block text-sm font-medium text-gray-700`}>
                            Segment title
                        </label>
                        <input id="segmentTitle" type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Enter a title" className={`flex-1 block w-full p-2 mt-1 rounded-xl outline-none transition border !border-gray-300 bg-white text-gray-900`} onKeyDown={(e) => e.key === 'Enter' && handleSave()} />
                    </div>
                </div>
            </Modal.Body>

            <Modal.Footer className={`flex items-center justify-end gap-3`}>
                <button type="button" className={`flex items-center justify-center gap-2 rounded-md px-3 py-2 w-fit transition hover:bg-light-hover-100`} onClick={onHide}>
                    <span className={`select-none font-medium text-textColor-300`}>Cancel</span>
                </button>

                <button type="button" className={`flex items-center justify-center gap-2 rounded-md px-3 py-2 w-fit transition ${!isTitleValid || isLoading ? 'cursor-not-allowed text-gray-400' : 'hover:bg-purple-50 text-purple-600'}`} onClick={handleSave} disabled={!isTitleValid || isLoading}>
