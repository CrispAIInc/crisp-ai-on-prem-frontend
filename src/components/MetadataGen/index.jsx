...
function MetadataGen() {
    ...
    async function generateMetadata() {
        ...
        try {
            ...
            // };
            const payload = {
                category: getCategories(displayedSources.filter(item => item.is_selected)), sources: displayedSources.filter(item => item.is_selected).map(source => ({ file_type: source.file_type, source_path: source.source_path })), selectedOptions: selectedOptions.map(op => op.id), inputContext: context, verbosityValue: verbosityValue
            };
