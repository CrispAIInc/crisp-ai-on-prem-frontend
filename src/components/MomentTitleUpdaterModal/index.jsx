...
const MomentTitleUpdaterModal = ({ show, onHide, moment, setMoments }) => {
    ...
    const isTitleValid = newMomentTitle.trim().length > 0;
            ...
            <Modal.Header className={`border-0 pb-0 ${theme === 'dark' ? '!bg-textColor-300 !text-white' : ''}`}>
                <div className="flex flex-col gap-1">
                    ...
                    <p className={`text-sm m-0 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>
                        Choose a clear title that helps describe this moment.
                    </p>
                </div>
            </Modal.Header>
            ...
            <Modal.Body className={`${theme === 'dark' ? 'bg-textColor-300 text-white' : ''}`}>
                <div className='flex flex-col items-start justify-center gap-3'>
                    <div className="flex flex-col w-full">
                        <label htmlFor="momentTitle" className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                            Moment title
                        </label>
                        <input
                            type="text"
                            name="momentTitle"
                            placeholder='Enter a new moment title'
                            id='momentTitle'
